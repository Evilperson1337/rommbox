...
namespace RomM.Platforms.Windows.Install
{
    internal sealed class ExecutableResolver
    {
        ...
        public ExecutableResolver(IPlatformLogger? logger)
        {
            _logger = logger;
        }
        ...
        public ExecutableResolutionResult Resolve(string installRoot, string? gameName, IReadOnlyCollection<string>? excludedRoots = null)
        {
            ...
            {
                _logger?.Write(PlatformLogLevel.Info, $"Executable resolver found single candidate '{executables[0]}'. Using without confirmation.");
            ...
            _logger?.Write(PlatformLogLevel.Info,
                $"Executable resolver found {executables.Count} candidates. Heuristic selected '{preferred}' (RootFirst={(preferredScore?.IsRoot == true)}, NameDistance={preferredScore?.NameDistance}, Arch={preferredScore?.Architecture}, ShortcutMatch={(preferredScore?.ShortcutMatch == true)}). Confirmation required.");
        ...
        private ExecutableResolutionResult ResolveFromManifest(string installRoot, string manifestPath)
        {
            try
            {
                ...
                {
                    _logger?.Write(PlatformLogLevel.Warning, $"Manifest executable not found. ManifestPath='{manifestPath}', RawExecutable='{manifest.Executable}', ResolvedExecutable='{resolvedExecutable}', Candidate='{candidate}'.");
                ...
                _logger?.Write(PlatformLogLevel.Info, $"Executable resolver selected manifest executable '{candidate}'. ManifestPath='{manifestPath}'.");
                if (resolvedArgs.Length > 0)
                {
                    _logger?.Write(PlatformLogLevel.Info, $"Manifest arguments resolved: {string.Join(" ", resolvedArgs)}");
                }
                return ExecutableResolutionResult.CreateSuccess(candidate, resolvedArgs);
            }
            catch (Exception ex)
            {
                _logger?.Write(PlatformLogLevel.Warning, "Failed to read manifest.json for executable resolution.", ex);
        ...
        private string? ResolveShortcutTarget(string installRoot, string normalizedGameName, IReadOnlyCollection<string> excludedRoots)
        {
            ...
            {
                _logger?.Write(PlatformLogLevel.Info, $"Executable resolver shortcut hint selected '{bestTarget}' from .lnk targets.");
            }
        ...
        private sealed class ManifestExecutable
        {
