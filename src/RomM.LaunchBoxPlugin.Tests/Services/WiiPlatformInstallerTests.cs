...
{
    public sealed class WiiPlatformInstallerTests
    {
        [Theory]
        [InlineData("game.iso")]
        [InlineData("game.wbfs")]
        [InlineData("game.rvz")]
        public async Task InstallAsync_DirectArtifact_InstallsToPerGameSubfolder(string fileName)
        {
            using var temp = new TempDirectory();
            var root = Path.Combine(temp.Path, "Games", "Nintendo Wii");
        ...
        public async Task InstallAsync_ArchiveWithExtractedArtifact_UsesInstalledArtifactForApplicationPath()
        {
            ...
            var image = Path.Combine(extracted, "game.wbfs");
            ...
            result.Success.Should().BeTrue();
            result.ExecutablePath.Should().Be(Path.Combine(root, "Game", "game.wbfs"));
        ...
        public async Task InstallAsync_MalformedArchiveWithoutExtractedContent_Fails()
        {
            ...
            result.Message.Should().Contain("Unable to resolve Nintendo Wii launch artifact");
        }
        ...
        {
            using var temp = new TempDirectory();
            var platformDir = Path.Combine(temp.Path, "Games", "Nintendo Wii");
        ...
        {
            using var temp = new TempDirectory();
            var installRoot = Path.Combine(temp.Path, "Games", "Nintendo Wii");
