        ...
        public async Task<InstallResult> ExecuteAsync(InstallContext context, IProgress<InstallProgressEvent> progress, CancellationToken cancellationToken)
        {
            ...
            if (InstallDestinationService.IsWindowsPlatform(platform.Name))
            {
                ...
                var result = await _windowsSubsystem
                    .InstallAsync(context.ArchivePath, context.ExtractedPath, context.InstallDirectory, context.PlatformMapping, context.Game.Title, cancellationToken, extractionProgress, installProgress)
                ...
                context.InstallStateSnapshot.InstallRootPath = ResolveInstallRootPath(result, context.InstallDirectory);
