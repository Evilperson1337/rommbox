...
namespace RomM.Platforms.GameCube
{
    public sealed class GameCubePlatformInstaller : IPlatformInstaller, IPlatformInstallerMetadata, IPlatformInstallerIdentityMetadata
    {
        ...
        public string PlatformKey => "gamecube";
        public string DisplayName => "Nintendo GameCube";
        public IReadOnlyCollection<string>? SupportedPlatformIds => new[] { "ngc", "gamecube" };
        ...
        public Task<DetectionResult> DetectAsync(PlatformContext ctx, CancellationToken ct)
        {
            ...
            if (!File.Exists(installedPath))
            {
                ...
                result.Warnings = new List<DetectionWarning>
                {
                    new() { Code = "installed_missing", Message = "Installed Nintendo GameCube artifact not found on disk." }
                };
                return Task.FromResult(result);
            }
            ...
            if (!SupportedExtensions.Contains(extension))
            {
                ...
                result.Warnings = new List<DetectionWarning>
                {
                    new() { Code = "unsupported_extension", Message = $"Installed file extension '{extension}' is not supported for Nintendo GameCube detection." }
                };
                return Task.FromResult(result);
            }
        ...
        public Task<VerifyResult> VerifyAsync(VerifyContext ctx, CancellationToken ct)
        {
            ...
            {
                IsValid = valid,
                Message = valid ? "Nintendo GameCube install verified." : "Nintendo GameCube install missing on disk."
            });
        }
        ...
        public Task<InstallResult> InstallAsync(InstallContext ctx, IProgress<InstallProgress> progress, CancellationToken ct)
        {
            ...
            ct.ThrowIfCancellationRequested();
            progress?.Report(new InstallProgress("Installing", "Preparing Nintendo GameCube install...", 0));
            ctx.Logger?.Write(PlatformLogLevel.Info, "Nintendo GameCube plugin selected");
            ...
            if (string.IsNullOrWhiteSpace(installRoot))
            {
                ...
                return Task.FromResult(new InstallResult { Success = false, Message = "Nintendo GameCube install directory missing." });
            }

            if (!DolphinInstallHelpers.EnsureDirectoryWritable(installRoot, "Nintendo GameCube", ctx.Logger, out var writableError))
            {
            ...
            if (!inspection.IsValid)
            {
                ...
                {
                    message = "Unable to resolve Nintendo GameCube launch artifact.";
                }

                return Task.FromResult(new InstallResult { Success = false, Message = message ?? "Nintendo GameCube inspection failed." });
            }

            var sourcePath = inspection.LaunchArtifactPath;
            if (string.IsNullOrWhiteSpace(sourcePath) || !File.Exists(sourcePath))
            {
                return Task.FromResult(new InstallResult { Success = false, Message = "Nintendo GameCube launch artifact missing on disk after inspection." });
            }

            var extension = Path.GetExtension(sourcePath) ?? string.Empty;
            if (!SupportedExtensions.Contains(extension))
            {
                return Task.FromResult(new InstallResult { Success = false, Message = $"Resolved GameCube launch artifact has unsupported extension '{extension}'." });
            }
            ...
            ctx.Logger?.Write(PlatformLogLevel.Info, $"Resolved GameCube install directory: {targetDirectory}");
            ctx.Logger?.Write(PlatformLogLevel.Info, $"Installing artifact: {sourcePath}");

            try
            {
                progress?.Report(new InstallProgress("Installing", "Installing Nintendo GameCube artifact...", 50));
                DolphinInstallHelpers.MoveOrReplace(sourcePath, targetPath, "Nintendo GameCube", ctx.Logger);
            }
            catch (Exception ex)
            {
                ctx.Logger?.Write(PlatformLogLevel.Warning, $"Nintendo GameCube install failed: {ex.Message}");
            ...
            {
                Success = true,
                Message = "Nintendo GameCube install completed.",
        ...
        public Task<UninstallResult> UninstallAsync(UninstallContext ctx, IProgress<InstallProgress> progress, CancellationToken ct)
        {
            ...
            ct.ThrowIfCancellationRequested();
            progress?.Report(new InstallProgress("Uninstall", "Nintendo GameCube uninstall started.", 0, true));
            ctx.Logger?.Write(PlatformLogLevel.Info, "Nintendo GameCube uninstall started");

            var removed = 0;
            var notes = new List<string>();
            removed += DolphinInstallHelpers.DeleteInstalledArtifactOnly(ctx.InstalledPath, "Nintendo GameCube", ctx.Logger, notes);
            ...
            {
                Success = true,
                Message = "Nintendo GameCube uninstall completed.",
